...
extension NSManagedObject {
    ...
    func attributeDescription(
        ...
        for (key, value) in entity.attributesByName {
            ...
            let customRemoteKey = entity.propertiesByName[value.name]?.customKey() ?? ""
            ...
            if currentAttributeHasTheSameRemoteKey {
                foundAttributeDescription = value
            }
        ...
        if foundAttributeDescription == nil {
            for (key, value) in entity.attributesByName {
                if (remoteKey == SyncDefaultRemotePrimaryKey) && (key == SyncDefaultLocalPrimaryKey || key == SyncDefaultLocalCompatiblePrimaryKey) {
                    foundAttributeDescription = value
                }
            }
        }
    ...
    func attributeDescriptions(forRemoteKeyPath remoteKey: String?) -> [AnyHashable]? {
        ...
        for (key, value) in entity.attributesByName {
            //let customRemoteKeyPath = entity.propertiesByName[value.name]?.customKey() ?? ""
            let customRootRemoteKey = key.components(separatedBy: ".").first
            ...
            if currentAttributeHasTheSameRootRemoteKey {
                foundAttributeDescriptions.append(value)
            }
        }
    ...
    func value(
        ...
        if type(of: remoteValue) == type(of: attributedClass) {
            value = remoteValue
        }
