        ...
        for (key, value) in dictionary {
            ...
            if let attributeDescription = attributeDescription {
                ...
                if valueExists && (value is [AnyHashable : Any]) && attributeDescription.attributeType != .binaryDataAttributeType {
                    ...
                    let hasCustomKeyPath = remoteKey != nil && (remoteKey! as NSString).range(of: ".").location != NSNotFound
                    if hasCustomKeyPath {
                        ...
                        if let keyPathAttributeDescriptions = attributeDescriptions(forRemoteKeyPath: remoteKey) {
                            for keyPathAttributeDescription in keyPathAttributeDescriptions {
                                ...
                                let valuex = dictionary.first { k, v in
                                    k == remoteKey
                                }?.value
            ...
            if propertyDescription is NSAttributeDescription {
                if propertyDescription.shouldExportAttribute() {
                    ...
                    if let value = value {
                        ...
                        let components = (split! as NSArray).subarray(with: range)
                ...
                if ((relationshipDescription?.shouldExportAttribute()) != nil) {
                    let isValidRelationship = !(parent != nil && (parent?.entity == relationshipDescription?.destinationEntity) && !(relationshipDescription?.isToMany ?? false))
                    if isValidRelationship {
                        ...
                        if let relationships = relationships {
                            ...
                            if isToOneRelationship {
                                var attributesForToOneRelationship: [AnyHashable : Any]? = nil
                                ...
                                if attributesForToOneRelationship != nil {
                                    for (k, v) in attributesForToOneRelationship! { managedObjectAttributes[k] = v }
                                }
                                                                
                            } else {
                                var attributesForToManyRelationship: [AnyHashable : Any]? = nil
                                ...
                                if attributesForToManyRelationship != nil {
